...
void IVExtraction(char *stringIV, uint8_t IV[16]){
    int size = 0;
    int index = 0;
    char *currentHex = malloc(sizeof(char) * 3);  
    char *end = "\0";
    ...
    if(size < 16 || (stringIV[index] != '\0' && stringIV[index + 1] != '\0')){
        free(currentHex);
        fprintf(stderr,"The IV must be of size 128 bits\n");
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv){
    ...
    */
    opt = getopt_long(argc, argv, "+hdi:k:o:m:I:", long_options, &option_index);
    while (opt != -1) {
        switch (opt) {
        case 'h':
            free(key);
